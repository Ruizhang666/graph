...
if G.number_of_nodes() == 0:
    print("图为空，无法进行高级分析。请检查CSV文件和graph_builder.py的输出。")
    ...
    for node, centrality in top_degree:
        print(f"{G.nodes[node]['name']}: {centrality:.4f}")
    ...
    try:
        ...
        for i, (community_id, members) in enumerate(largest_communities):
            ...
            for j, node in enumerate(members[:5]):  # 只显示前5个成员
                print(f"  - {G.nodes[node]['name']}")
            if len(members) > 5:
                print(f"  - ... 以及其他 {len(members) - 5} 个成员")
        ...
        plt.axis('off')
        plt.tight_layout()
        plt.savefig('股权关系社区结构.png', dpi=300, bbox_inches='tight')
        
    except ImportError:
        print("无法进行社区检测，请安装python-louvain库: pip install python-louvain")
        ...
        for i, cycle in enumerate(cycles[:5]):
            ...
            for j in range(len(cycle)):
                ...
                if G.has_edge(source, target):
                    ...
                    print(f"  {G.nodes[source]['name']} -> {G.nodes[target]['name']}: {percent_str}")
            
            # 如果成功计算了所有百分比，显示总体控制比例
            if isinstance(total_control, float):
                print(f"  循环控制比例: {total_control:.6f} ({total_control*100:.4f}%)")
    else:
        print("未发现循环持股关系")
    ...
    for i, (node, degree) in enumerate(top_controllers[:3]):
        if degree > 0:  # 确保有出边
            ...
            for j, (entity_id, entity_name, percent) in enumerate(controlled_entities[:5]):
                print(f"  - {entity_name}: {percent}")
            
            if len(controlled_entities) > 5:
                print(f"  - ... 以及其他 {len(controlled_entities) - 5} 个实体")
    ...
    print(f"发现 {len(ultimate_controllers)} 个潜在的最终控制人")
    ...
    for i, controller in enumerate(ultimate_controllers[:5]):  # 只显示前5个
        ...
        if reachable:
            ...
            for j, node in enumerate(list(reachable)[:3]):
                print(f"    - {G.nodes[node]['name']}")
            if len(reachable) > 3:
                print(f"    - ... 以及其他 {len(reachable) - 3} 个实体")
    ...
    if ultimate_controllers:
        ...
        plt.axis('off')
        plt.tight_layout()
        plt.savefig('最终控制人网络图.png', dpi=300, bbox_inches='tight')
    ...
    if self_loops:
        ...
        for node_id in self_loops[:5]: # 显示前5个
            print(f"  - 节点 '{G.nodes[node_id].get('name', node_id)}' 指向自身")
    else:
        print("\n检查通过：未发现自循环。")
    ...
    if parents_missing_names:
        ...
        for p_id in list(set(parents_missing_names))[:5]: # 显示前5个独特的父节点ID
            ...
            print(f"  - 父节点ID '{p_id}' (名称缺失) 投资了: {children_names[:3]}")
    else:
